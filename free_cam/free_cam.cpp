...
void scan_api_signature() {
    wcout << GetTimestamp() << L" 开始扫描游戏版本号，特征码: " << Constants::Game_Build_Version_STR.c_str() << endl;
    auto start_time = chrono::steady_clock::now();

    auto api_scan_result = AOBScan(hProcess, Constants::Game_Build_Version_STR);

    // 检查扫描结果并读取内存
    if (api_scan_result.address) {
        char buffer[256];  // 缓冲区，读取最多256字节
        SIZE_T bytes_read = 0;  // 初始化为0
        const SIZE_T buffer_size = sizeof(buffer);  // 缓冲区大小

        if (ReadProcessMemory(hProcess, (LPCVOID)api_scan_result.address, buffer, buffer_size, &bytes_read)) {
            // 确保不会越界，只在 bytes_read < buffer_size 时添加空字符
            if (bytes_read > 0 && bytes_read < buffer_size) {
                buffer[bytes_read] = '\0';  // 安全添加空字符
            }
            else if (bytes_read == buffer_size) {
                buffer[buffer_size - 1] = '\0';  // 如果正好满，覆盖最后一个字符
            }
            else {
                buffer[0] = '\0';  // 没有读取到数据，设置为空字符串
            }
            std::string result_str(buffer);  // 转换为 string 类型
            wcout << GetTimestamp() << L" 游戏版本: " << std::wstring(result_str.begin(), result_str.end()).c_str() << endl;
        }
        else {
            wcout << GetTimestamp() << L" 读取内存失败，错误代码: " << GetLastError() << endl;
        }
    }
    else {
        wcout << GetTimestamp() << L" 游戏版本号特征码扫描失败，未找到匹配地址" << endl;
    }

    auto end_time = chrono::steady_clock::now();
    auto duration = chrono::duration_cast<chrono::milliseconds>(end_time - start_time).count();
    wcout << GetTimestamp() << L" API特征码扫描结束，地址: 0x" << hex << api_scan_result.address
...
void run_main_loop() {
    ...
    auto last_scan_time = chrono::steady_clock::now();
    ...
    while (true) {
        ...
        if (chrono::duration_cast<chrono::milliseconds>(now - last_read_time).count() >= 100) {
            ...
            // 检查 local_player 是否既不是 0 也不是 1
            if (local_player != 0 && local_player != 1) {
                if (chrono::duration_cast<chrono::milliseconds>(now - last_scan_time).count() >= 1000) { // 限制扫描频率
                    wcout << GetTimestamp() << L" 检测到异常 local_player 值: " << local_player
                        << L"，重新扫描指针" << endl;

                    // 重新扫描指针
                    DWORD selected_pid = GetProcessId(hProcess);
                    if (!initialize_pointers(selected_pid)) {
                        wcout << GetTimestamp() << L" 指针重新扫描失败" << endl;
                        ProcessManager::CloseProcessHandle(hProcess);
                        return;
                    }
